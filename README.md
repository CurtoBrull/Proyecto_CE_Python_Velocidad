...
El código es idéntico al Detector 1, excepto que envía `{"detector2": timestamp_ms}` en lugar de `detector1`.

Ver archivo completo en: `placa/detector2.py`
...
1. Configurar WiFi en `placa/detector1.py` y `placa/detector2.py`:

```python
# Cambiar estas líneas en ambos archivos
sta_if.connect('TuWiFi', 'TuPassword')
url_servicio = "https://radarpythonapi.onrender.com/mediciones/"  # Para Render
# O para uso local:
# url_servicio = "http://TU_IP:8080/mediciones/"
```

2. Cargar código a ambos ESP32 (ver sección 5.4.3)

3. Colocar los dos ESP32 separados 1-2 metros (cada uno con su sensor PIR)

4. Pasar la mano/objeto frente al primer ESP32 (detector1)
   - Este enviará `{"detector1": timestamp}`

5. Pasar la mano/objeto frente al segundo ESP32 (detector2)
   - Este enviará `{"detector2": timestamp}`
   - La API calculará la velocidad automáticamente
